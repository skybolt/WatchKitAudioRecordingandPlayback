...
- (void)viewDidLoad {
    ...
    //get boolean from WCSessionIsSupported and pass it to a label or something
    //if this is all message based, and it probably is, then perhaps you can
    //supress a label with a timer that resets every time the message is recieved
    //then if the timer reaches a certain value, the implication is that no expected
    //message has been recieved, therefore the watch must not be connected
    //and therefore we can alert "watch out of range" or something
    
    //forget ALL THAT and use this instead: (assuming you can derive the syntax)
    //- sessionReachabilityDidChange: //(New in watchOS 2.0)
